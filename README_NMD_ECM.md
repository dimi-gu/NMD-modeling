...
The notebook models the rate paid on NMDs as a function of market rates and additional economic drivers.
...
+\beta r_t^m
+\phi_s S_t
+\phi_c D_t^{\mathrm{COVID}}
+\varepsilon_t
\]

where:

- \(r_t\) is the NMD customer rate;
- \(r_t^m\) is the one-month Euribor rate;
- \(S_t\) is the Italian–German 10-year sovereign spread;
- \(D_t^{\mathrm{COVID}}\) is a COVID-period dummy;
- \(\beta\) measures long-run market-rate pass-through;
- \(\varepsilon_t\) is the deviation from long-run equilibrium.

The initial candidate regressors are:
...
The constant is retained throughout the selection process.

The notebook expects positive coefficients for:

- `Euribor_1M`;
- `Spread`.

A variable is removed when its coefficient has the wrong expected sign or its p-value exceeds the selected significance level.
...
- \(\theta\) is the speed of adjustment toward long-run equilibrium;
- \(\gamma^+\) measures the immediate response to positive market-rate changes;
- \(\gamma^-\) measures the immediate response to negative market-rate changes;
- \(r_{t-1}-\widehat r_{t-1}\) is the lagged long-run residual.

The theoretical ECM restriction applied to the adjustment coefficient is:

\[
-1 \leq \theta \leq 0
\]

The notebook tests progressively simpler short-run structures:

| Step | Regressors | Interpretation |
|---:|---|---|
| 0 | `theta`, `gamma_positive`, `gamma_negative` | Fully asymmetric pass-through |
| 1 | `theta`, `gamma_negative` | Downward adjustment only |
| 2 | `theta`, `gamma` | Symmetric pass-through |
| 3 | `theta` | Error correction without immediate pass-through |

Gamma terms are assessed using significance tests and restrictions relative to the long-run pass-through coefficient \(\beta\).

---

## Mean-Reversion Time

The notebook derives the estimated time required for the customer rate to converge toward the market-rate shock.
...
Because the input observations are monthly, the resulting mean-reversion time is expressed in months.
